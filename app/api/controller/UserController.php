...
class UserController extends AuthController
{
    public function userInfo()
    {
        ...
        $user = User::where('id', $user['id'])->field('id,phone,realname,pay_password,up_user_id,is_active,invite_code,ic_number,level,balance,topup_balance,poverty_subsidy_amount,digital_yuan_amount,can_open_digital,team_bonus_balance,created_at,qq,avatar,signin_balance,life_balance')->find()->toArray();
...
/*     public function hongbao(){
    ...
    public function wallet(){
        ...
        ->where('log_type',1)->whereIn('type',[1,2,3,13,18,19,30,31,32,4,5,6,7,8])
...
/*         if($log_type == 1)
    ...
    public function teamInfo()
    {
        $user = $this->user;
        $return = [];
        $sonIds = UserRelation::where('user_id', $user['id'])->whereIn('level', [1, 2, 3])->column('sub_user_id');
        ...
        //新增人数
        $return['addCount'] = UserRelation::where('user_id', $user['id'])->where('level', 1)->where('created_at', '>=', date('Y-m-d 00:00:00'))->count();
     ...
     * 团队人数-推广奖励明细
     */
    public function layerInfoReward()
    {
        $user = $this->user;
        //$sonIds = UserRelation::where('user_id', $user['id'])->whereIn('level', [1, 2, 3])->column('sub_user_id');
        $list = UserBalanceLog::field('id, change_balance, created_at, origin_pay, user_id, relation_id')->where('type', 8)->where('user_id', $user['id'])->order('id', 'desc')->paginate();
        foreach ($list as $value) {
            $value['user_name'] = User::where('id', $value['relation_id'])->value('realname');
            $value['level'] = UserRelation::where('user_id', $user['id'])->where('sub_user_id', $value['relation_id'])->value('level');
        }
        return out($list);
    }
    ...
    public function assets($userId)
    {
        ...
        $assets += Db::name('order')->where('user_id', $userId)->where('status', 1)->sum('buy_amount');
    ...
    public function mine()
    {
        ...
        //总资产
        $return['totalAssets'] = $return['balance'] + $return['specific_balance'] + $return['teamIncome'];
        //邀请码
