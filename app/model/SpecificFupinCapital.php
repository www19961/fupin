...
class SpecificFupinCapital extends Model
{
    ...
    {
        $capital = SpecificFupinCapital::where('id', $id)->lock(true)->find();
...
/*         if ($capital['pay_channel'] == 1 && $status == 2) {
        ...
        SpecificFupinCapital::where('id', $id)->update($update);
        ...
        if ($status != 4) {
            ...
            if ($capital['pay_channel'] == 1) {
                ...
                if ($status == 3) {
                    ...
                    User::changeInc($capital['user_id'], $change,'specific_fupin_balance', 40, $id, 3, $audit_remark ?? '', $admin_user_id,1,'TX');
                }
                else {
                    // 审核通过把资金日志的提现记录变为已完成
                    UserBalanceLog::where('user_id', $capital['user_id'])->where('type', 40)->where('relation_id', $id)->where('log_type', 3)->where('status', 1)->update(['status' => 2]);
                }
            //}
        }
        else {
