...
use app\model\GoldEggPrizeUserLog;
use app\model\GoldEggPrize;
...
class GoldEggController extends AuthController
{
    ...
    {
        $this->assign('data', GoldEggPrize::select()->toArray());
        return $this->fetch();  
    }
    ...
    public function prizeSetting()
    {
        ...
        $data = [];
        if (!empty($req['id'])) {
            $data = GoldEggPrize::where('id', $req['id'])->find();
        }
    ...
    public function editConfig()
    {
        ...
        GoldEggPrize::where('id', $req['id'])->update($req);
    ...
    //砸金蛋记录
    public function GoldEggPrizeUserLog()
    {
        $req = request()->param();
        $builder = GoldEggPrizeUserLog::alias('l')->field('l.*, p.name, u.phone');
