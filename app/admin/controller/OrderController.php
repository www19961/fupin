...
class OrderController extends AuthController
{
    public function orderList()
    {
        ...
        if (!empty($req['channel'])||!empty($req['mark'])) {
            ...
            $builder = Order::alias('o')->order('o.id', 'desc');
        }
        ...
        if (isset($req['project_name']) && $req['project_name'] !== '') {
            $builder->whereLike('pro.name', '%'.$req['project_name'].'%');
        }
        ...
        $total_buy_integral = round($builder2->sum('o.buy_num'), 2);
        $this->assign('total_buy_integral', $total_buy_integral);

        $builder3 = clone $builder;
        $total_gift_equity = round($builder3->sum('o.buy_num'), 2);
        $this->assign('total_gift_equity', $total_gift_equity);

        $builder4 = clone $builder;
        $total_gift_digital_yuan = round($builder4->sum('o.buy_num'), 2);
